...
@implementation CYVideoPlayerMoreSetting
...
- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image clickedExeBlock:(void(^)(CYVideoPlayerMoreSetting *model))block {
    return [self initWithTitle:title image:image showTowSetting:NO twoSettingTopTitle:@"" twoSettingItems:@[] clickedExeBlock:block];
}

- (instancetype)initWithTitle:(NSString *)title image:(UIImage *)image showTowSetting:(BOOL)showTowSetting twoSettingTopTitle:(nonnull NSString *)twoSettingTopTitle twoSettingItems:(nonnull NSArray<CYVideoPlayerMoreSettingSecondary *> *)items clickedExeBlock:(nonnull void (^)(CYVideoPlayerMoreSetting * _Nonnull))block {
    self = [super init];
    if ( !self ) return self;
    self.title = title;
    self.image = image;
