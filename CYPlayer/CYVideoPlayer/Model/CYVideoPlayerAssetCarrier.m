...
@interface CYTmpObj : NSObject
@property (nonatomic, copy) void(^deallocCallBlock)(CYTmpObj *obj);
@end

@implementation CYTmpObj
- (void)dealloc {
...
@interface CYVideoPlayerAssetCarrier () {
    id _timeObserver;
    id _itemEndObserver;
}
...
@implementation CYVideoPlayerAssetCarrier
...
- (void)_observing {
    ...
    if ( _scrollView ) {
        __weak typeof(self) _self = self;
        [self _observeScrollView:_scrollView deallocCallBlock:^(CYTmpObj *obj) {
            __strong typeof(_self) self = _self;
            if ( !self ) return;
    ...
    if ( _rootScrollView ) {
        __weak typeof(self) _self = self;
        [self _observeScrollView:_rootScrollView deallocCallBlock:^(CYTmpObj *obj) {
            __strong typeof(_self) self = _self;
            if ( !self ) return;
...
#pragma mark -
- (void)generatedPreviewImagesWithMaxItemSize:(CGSize)itemSize completion:(void (^)(CYVideoPlayerAssetCarrier * _Nonnull, NSArray<CYVideoPreviewModel *> * _Nullable, NSError * _Nullable))block {
    ...
    __weak typeof(self) _self = self;
    NSMutableArray <CYVideoPreviewModel *> *imagesM = [NSMutableArray new];
    ...
    [_imageGenerator generateCGImagesAsynchronouslyForTimes:timesM completionHandler:^(CMTime requestedTime, CGImageRef  _Nullable imageRef, CMTime actualTime, AVAssetImageGeneratorResult result, NSError * _Nullable error) {
        ...
        if ( result == AVAssetImageGeneratorSucceeded ) {
            UIImage *image = [UIImage imageWithCGImage:imageRef];
            CYVideoPreviewModel *model = [CYVideoPreviewModel previewModelWithImage:image localTime:actualTime];
...
- (void)_updateScrollView {
    ...
    // add observer
    __weak typeof(self) _self = self;
    [self _observeScrollView:newScrollView deallocCallBlock:^(CYTmpObj *obj) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
        if ( !self.removedScrollObserver ) {
            [self _removingScrollViewObserver];
...
- (void)_observeScrollView:(UIScrollView *)scrollView deallocCallBlock:(void(^)(CYTmpObj *obj))block {
    ...
    [self _injectTmpObjToScrollView:scrollView deallocCallBlock:^(CYTmpObj *obj) {
        obj.deallocCallBlock = block;
    }];
}

- (void)_injectTmpObjToScrollView:(UIScrollView *)scrollView deallocCallBlock:(void(^)(CYTmpObj *obj))block {
    CYTmpObj *obj = [CYTmpObj new];
    obj.deallocCallBlock = block;
