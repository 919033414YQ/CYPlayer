...
NSNotificationName const CY_AVPlayerRateDidChangeNotification = @"CY_AVPlayerRateDidChangeNotification";

/*!
 *  Refresh interval for timed observations of AVPlayer
 */
#define CYREFRESH_INTERVAL (0.5)

/*!
 *  0.0 - 1.0
 */
#define CYPreImgGenerateInterval (0.05)
...
- (void)_addTimeObserver {
    CMTime interval = CMTimeMakeWithSeconds(CYREFRESH_INTERVAL, NSEC_PER_SEC);
...
- (void)generatedPreviewImagesWithMaxItemSize:(CGSize)itemSize completion:(void (^)(CYVideoPlayerAssetCarrier * _Nonnull, NSArray<CYVideoPreviewModel *> * _Nullable, NSError * _Nullable))block {
    ...
    if ( CYPreImgGenerateInterval > 1.0 || CYPreImgGenerateInterval <= 0 ) return;
    __block short maxCount = (short)floorf(1.0 / CYPreImgGenerateInterval);
    short interval = (short)floor(seconds * CYPreImgGenerateInterval);
...
- (void)_injectTmpObjToScrollView {
    ...
    __weak typeof(self) _self = self;
    obj.deallocCallBlock = ^(CYTmpObj *obj) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
