...
@interface CYLineModel : NSObject
...
@interface CYCTData ()
@property (nonatomic, strong, readonly) NSMutableArray<CYLineModel *> *drawingLinesM;
@end

@implementation CYCTData
...
//    CGFloat lineH = ABS(_config.font.descender) + _config.font.ascender + _config.font.leading + _config.lineSpacing;
    for ( CFIndex lineIndex = 0 ; lineIndex < numberOfLines ; lineIndex ++ ) {
        ...
        CYLineModel *recordLine = [CYLineModel new];
    ...
    if ( 0 != _config.numberOfLines && _config.numberOfLines < lines ) {
        ...
        CYLineModel *recordLine = _drawingLinesM.lastObject;
        ...
        [_drawingLinesM enumerateObjectsUsingBlock:^(CYLineModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        [_imageDataArray enumerateObjectsUsingBlock:^(CYCTImageData * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)drawingWithContext:(CGContextRef)context {
    ...
    for ( CYCTImageData *imageData in _imageDataArray ) {
    ...
    [_drawingLinesM enumerateObjectsUsingBlock:^(CYLineModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ( point.y > head && point.y < tail ) {
            *stop = YES;
            ...
            [_attrStr enumerateAttribute:CYActionAttributeName inRange:NSMakeRange(0, _attrStr.length) options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired usingBlock:^(id  _Nullable value, NSRange range, BOOL * _Nonnull stop) {
